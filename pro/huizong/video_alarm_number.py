...
def get_cdn_hostgroup_id():
    cursor = conn.cursor()
    cursor.execute("select id from grp where grp_name like \'%视频云-S3andVaas%\'")
...
def get_cdn_endpoints():
    ...
    cursor.execute("select distinct(hostname) from host join grp_host on \
            host.id=grp_host.host_id join grp on grp_host.grp_id=grp.id where grp.grp_name like \'%视频云%\'")
    rows = cursor.fetchall()
    cursor.close()
    return map(lambda x: x[0], list(rows))

def write_alarm(date_str, data):
    filename = os.path.join(ALARM_LOG_PATH, "video_alarm-%s.log" % date_str)
    f = open(filename, 'w')
...
if __name__ == '__main__':
    ...
    for i in range(5):
        ...
        alarm_log_file = os.path.join(ALARM_LOG_PATH, "video_alarm-%s.log" % date_str)
    ...
    sender = Sender()
    sender.EMAIL('chenchengsong@le.com,fanbaoliang@le.com,wangziyin@le.com,wangyanping@le.com,jialei@le.com,liubin5@le.com', 'Video Alarm Number', h)
    #sender.EMAIL('iops-cdn@le.com', 'Video Alarm Number', h)