...
PUSH_GATEWAY = "36.110.161.36:9091"
...
def handle_key(logfile, fd, **kw):
    ...
    p_pull = re.compile(r'Time:(\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}:\d{2}),\s+.*Stream:(.*),\s+ClientIP:(\d+\.\d+\.\d+\.\d+)_(\d+),.*DistributeRate:(.*),\s')
    ...
    if file_size > 0:
        ...
        push_to_gateway(PUSH_GATEWAY, job="live-rtmpstream-status", registry=registry)
