...
sql1 = 'select mean(usage_user) as %sused from %s where time >= %s000ms and time <= %s000ms group by host;'%(value,value,tday1,tday7)
sql2 = 'select max(usage_user) as  %smax from %s where time >= %s000ms and time <= %s000ms group by host;'%(value,value,tday1,tday7)
sql = {
         '%used':sql1,
         '%smax':sql2
         }
